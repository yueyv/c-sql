...
namespace work
{
    public partial class Admin个人 : Form
    {
        string code;
        public Admin个人()
        {
        ...
        private void Admin个人_Load(object sender, EventArgs e)
        {
            load();
        }
        private void load()
        {
            Link da = new Link();
            string sql = $"select * from Admin where 管理员编号='{label2.Text}'";
        ...
        {
            string sql = $"update Admin set 姓名='{textBox1.Text}',性别='{textBox2.Text}',年龄='{textBox3.Text}',电话='{textBox4.Text}',住址='{textBox5.Text}'  where 管理员编号='{label2.Text}'";
            Link da = new Link();
            if (da.Excute(sql) > 0)
            {
        ...
        {
            Admin个密 a = new Admin个密(label2.Text,code);
            this.Hide();
            a.ShowDialog();
            this.Show();
        }
